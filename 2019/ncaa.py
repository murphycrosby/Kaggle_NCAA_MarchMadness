...
def load_historical_data(season_year, onehot_season, onehot_daynum, onehot_teams):
    ...
    args = []
    for x in df_split:
        args.append([x, full, onehot_season, onehot_daynum, onehot_teams])
...
def parse_stats_data(df, full, onehot_season, onehot_daynum, onehot_teams):
    ...
    for index, row in df.iterrows():
        ...
        w_hist = full[(full.Season == row.Season) & (full.DayNum < row.DayNum) &
            ...
            .sort_values(by=['DayNum'], ascending=False).head(3)
...
def predict_stage():
    ...
    for season in range(2019, 2020):
        ...
        for m in matchups:
            ...
            x, y = parse_stats_data(df, full, onehot_season, onehot_daynum, onehot_teams)
            x = x.reshape(1, 3, 1860)
...
def train_model():
    ...
    if model_file.exists():
        ...
        #
        model.add(LSTM(64, input_shape=(3, 1860), activation='softmax', return_sequences=True))
    ...
    model.fit(x_train, y_train, epochs=25, batch_size=20, verbose=1, callbacks=[es, mc])
...
def process_historical():
    ...
    for season in range(2019, 2020):
        ...
        load_historical_data(season, onehot_season, onehot_daynum, onehot_teams)
