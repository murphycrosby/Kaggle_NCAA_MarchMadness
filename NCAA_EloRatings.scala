...
def eloDataframeToMap(df:DataFrame): Map[Int, Double] = {
  var elo = Map[Int, Double]()
  for(row <- df.collect()) {
...
def set_elo(teamDF:DataFrame, eloMap:Map[Int, Double], init:Boolean): Map[Int, Double] = {
  var team_elo = Map[Int, Double]()
...
def calc_elo(gameRow:Row, elo_map:Map[Int, Double]) : (Double,Double,Int,Double,Double,Int,Double,Double) = {
...
def calc_elo_year(season:Int,d_frame:DataFrame,elo_map:Map[Int, Double]) : (Map[Int, Double]) = {
  var map = elo_map
...
val coaches = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/TeamCoaches.csv")
val teams = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/Teams.csv")
val cities = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/Cities.csv")
val game_cities = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/GameCities.csv")
val ncaaresults = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/NCAATourneyDetailedResults.csv")
val regresults = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load("dbfs:/FileStore/tables/RegularSeasonDetailedResults.csv")

// COMMAND ----------

val games_1 = regresults.union(ncaaresults)
...
val games_3 = (games_2.join(coaches, games_2.col("Season") === coaches.col("Season")
                           && games_2.col("WTeamID") === coaches.col("TeamID")
                           && games_2.col("DayNum") >= coaches.col("FirstDayNum")
                           && games_2.col("DayNum") <= coaches.col("LastDayNum")
                           ,"left")
...
val games_5 = games_4.orderBy($"Season".asc, $"DayNum".asc)
...
//2018
val games_2018 = regresults_2018.filter("Season = 2018")
